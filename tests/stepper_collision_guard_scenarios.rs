...
#[test]
fn stepper_collision_guard_scenarios_cover_nominal_and_fault_paths() {
    let plc = repo_path("examples/stepper_collision_guard.plc");
    assert!(plc.exists(), "expected PLC example to exist");

    let scenario_dir = repo_path("scenarios/stepper_collision_guard");
    let cases = [
        ("normal.yaml", false),
        ("count_stuck.yaml", true),
        ("wrong_direction.yaml", true),
        ("alarm_triggered.yaml", true),
    ];

    let base = temp_dir("rust_plc_stepper_collision_guard");
    ...
    assert_eq!(
        timeout_case_count, 3,
        "expected three fault scenarios to hit timeout path"
    );
}
