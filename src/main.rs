...
fn run_new_subcommand(program: &str, mut args: impl Iterator<Item = String>) -> Result<(), String> {
    ...
    let readme = "# RustPLC Bootstrap Project\n\n## Quick Start Checklist\n\n1. Validate scenario contract:\n\n```bash\ncargo run --release --bin rust_plc -- scenario-validate plc/main.plc --scenario scenarios/normal.yaml --output human\n```\n\n2. Run diagnostic pre-check (`scenario-doctor`):\n\n```bash\ncargo run --release --bin rust_plc -- scenario-doctor plc/main.plc --scenario scenarios/normal.yaml --output human\n```\n\n3. Run no-board regression gate:\n\n```bash\ncargo run --release --bin rust_plc -- no-board-gate plc/main.plc --scenario scenarios/normal.yaml --out-dir out/no_board_gate --output human\n```\n\n4. Optional RP2040 build baseline:\n\n```bash\ncargo run --release --bin rust_plc -- build-rp2040 plc/main.plc --out out/rp2040 --io-map io_map.toml\n```\n\n## VS Code\n\n- Open Command Palette and run `Tasks: Run Task`.\n- Use prefixed tasks (`RustPLC: ...`) from `.vscode/tasks.json`.\n- See `.vscode/README.md` for troubleshooting.\n";
    ...
    let workflow = "name: rustplc-no-board-gate\n\non:\n  push:\n  pull_request:\n\njobs:\n  no-board-gate:\n    runs-on: ubuntu-latest\n    steps:\n      - uses: actions/checkout@v4\n      - uses: dtolnay/rust-toolchain@stable\n      - name: Scenario validate\n        run: cargo run --release --bin rust_plc -- scenario-validate plc/main.plc --scenario scenarios/normal.yaml --output json\n      - name: No-board gate\n        run: cargo run --release --bin rust_plc -- no-board-gate plc/main.plc --scenario scenarios/normal.yaml --out-dir out/no_board_gate --output json\n";
    let vscode_tasks = "{\n  \"version\": \"2.0.0\",\n  \"tasks\": [\n    {\n      \"label\": \"RustPLC: scenario-init (normal)\",\n      \"type\": \"shell\",\n      \"command\": \"cargo run --release --bin rust_plc -- scenario-init plc/main.plc --preset normal --out scenarios/normal.yaml\",\n      \"problemMatcher\": []\n    },\n    {\n      \"label\": \"RustPLC: scenario-validate\",\n      \"type\": \"shell\",\n      \"command\": \"cargo run --release --bin rust_plc -- scenario-validate plc/main.plc --scenario scenarios/normal.yaml --output human\",\n      \"problemMatcher\": []\n    },\n    {\n      \"label\": \"RustPLC: scenario-doctor\",\n      \"type\": \"shell\",\n      \"command\": \"cargo run --release --bin rust_plc -- scenario-doctor plc/main.plc --scenario scenarios/normal.yaml --fix-preview --output human\",\n      \"problemMatcher\": []\n    },\n    {\n      \"label\": \"RustPLC: sim-plc\",\n      \"type\": \"shell\",\n      \"command\": \"cargo run --release --bin rust_plc -- sim-plc plc/main.plc --scenario scenarios/normal.yaml --out out/sim_plc/trace.jsonl\",\n      \"problemMatcher\": []\n    },\n    {\n      \"label\": \"RustPLC: no-board-gate\",\n      \"type\": \"shell\",\n      \"command\": \"cargo run --release --bin rust_plc -- no-board-gate plc/main.plc --scenario scenarios/normal.yaml --out-dir out/no_board_gate --output human\",\n      \"problemMatcher\": []\n    },\n    {\n      \"label\": \"RustPLC: build-rp2040\",\n      \"type\": \"shell\",\n      \"command\": \"cargo run --release --bin rust_plc -- build-rp2040 plc/main.plc --out out/rp2040 --io-map io_map.toml\",\n      \"problemMatcher\": []\n    }\n  ]\n}\n";
