...
3. If `docs/architecture/signal-direction.md` exists, read it before selecting a story
...
## Global Invariants (Every Iteration)

- Use the architecture definitions in `docs/architecture/signal-direction.md` as the single source of truth.
- Treat DSL fields as a strict whitelist schema: any unknown/non-spec field must fail compilation.
- Keep recurring workflow rules in this file (and `CODEX.md`), and keep each PRD story focused on incremental scope and acceptance.
...
- **Parser** (`src/parser/plc.pest` + `src/parser/mod.rs`): PEG 语法 → AST。长关键字必须在短前缀之前（如 `must_complete_within_worst_case` 在 `must_complete_within` 之前）。Wrapper 规则必须先解包再匹配具体规则。
