...
当前 `plc-gen` 已能先给出 lowering 摘要，再说明 `wafer_loader.system.md` 如何降为 `.plc` 里的并发 task、service task、warning/fault task、变量与共享资源结构。
...
1. 步骤：
   观察到的阻塞：无新的 confirmed-system lowering 阻塞。
   缺少的工件或说明：无。
   影响：满足停止条件。
