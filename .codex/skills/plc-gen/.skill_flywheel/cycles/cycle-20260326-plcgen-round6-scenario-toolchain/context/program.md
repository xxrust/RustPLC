...
`plc-gen` 当前真正缺的不是更多 reference，而是两个显式主路径：

- “已确认 `.system.md` -> scaffold -> `plc/main.plc` -> validation”的 Day-1 公开面
- “已确认 `.system.md` -> `.plc` lowering 摘要 -> `.plc` 结构”的降级规则
- “需要当前 scenario 工具链直接可用时 -> scenario-friendly lowering / toolchain blocker”的兼容性规则

如果把 launcher 选择、system contract gate、文件顺序、validation order、lowering 规则以及 scenario toolchain 兼容性规则都显式导出，并在 skill 本体里补齐“已给 `.system.md` 时不要重新发散需求”的规则，盲测执行者就更容易稳定交付真实 PLC 答案。
