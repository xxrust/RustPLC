...
真实命令链表明，复杂 PLC 在当前 scenario 工具链下会因复合 wait guard 触发 `unsupported guard expression`；这不是之前 skill 已显式处理的路径。
...
1. 步骤：
   观察到的阻塞：scenario 命令链被复合 guard 阻塞。
   缺少的工件或说明：`scenario-toolchain-limitations.md`
   影响：需要把 toolchain blocker 提升为公开决策点。
