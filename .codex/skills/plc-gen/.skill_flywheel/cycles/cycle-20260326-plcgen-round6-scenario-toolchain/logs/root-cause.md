...
1. 痛点：scenario 工具链对复杂 wait guard 的兼容性没有进入 skill 主路径
   分类：skill-gap
   原因：`plc-gen` 默认推荐 scenario 命令链，但没有先做 scenario-compatibility 检查，也没有要求 scenario-friendly lowering。
   最小修复：在 skill 中加入 scenario 兼容性检查和 scenario-friendly lowering 规则。

2. 痛点：真实错误信息没有进入公开面
   分类：public-surface-gap
   原因：执行者无法仅靠 skill 公开工件知道 `unsupported guard expression` 是当前工具链阻塞，而非 DSL 必然错误。
   最小修复：导出 `scenario-toolchain-limitations.md`，并同步 troubleshooting。
