...
- 复合 `wait: X0 == true AND X1 == true` 会卡 `scenario-init`
- 顺序单条件 `wait` 版本可通过 `scenario-init` 和 `scenario-validate`
...
1. 如果语义允许，先顺序拆成多个单条件 `wait`
2. 若不能顺序化，再尝试 helper readiness gate
3. 最后才考虑更复杂的中间 step 决策
