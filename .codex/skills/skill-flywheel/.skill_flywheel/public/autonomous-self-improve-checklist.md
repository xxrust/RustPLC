...
- 观察时优先直接使用 `autonomous-self-improve-observe-command.txt`，不要临时拼一套不一致的检查命令

## 运行中必须观察

1. `runner_state.json`
   - if `last_cycle` already points at a post-baseline cycle but `logs/decision.json` is still placeholder content, do not open a new cycle; close out that active cycle first via `public/single-agent-closeout-checklist.md`
   - `status` 初始应为 `active`
