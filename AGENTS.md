...
- Phase 3 safety verification 不能退回到类型计数近似：`split` 必须消费“全局恰好 1 个 active 的 `source_type` token”，`merge` 必须按声明的输入类型多重集消费彼此不同的 active token，终态 dangling 检查必须基于具体 active token 实例而不是 warning 级 type-count 估算
...
### 9. task 中的设备动作必须保持高层语义

- `task.step` 表达的是“设备要做什么”以及“不同结果如何分流”，不是手写传感器编排
- 对于拓扑已闭合的机构设备，动作成功、超时以及全部关联反馈导出的异常结果，必须由 semantic / IR / runtime 闭环建模
- 这里的“拓扑已闭合”指：`device` / library / topology 已经形式化给出“设备动作 -> 关联反馈集合 -> 结果枚举”的可判定映射；如果做不到这一点，就视为未闭合
- 最小结果集合不是自由发挥，而是要从该动作关联反馈的组合中穷举出来：正常确认组合对应成功，超时对应未到位，其余反馈组合必须映射成显式异常结果
