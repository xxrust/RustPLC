...
- Built-in math externs (`add`/`multiply`/`quadratic_fit`) are bootstrapped by `ExternFunctionRegistry::new`; tests using `with_time_source` must call `register_builtin_math_functions()` explicitly when they need built-ins.
