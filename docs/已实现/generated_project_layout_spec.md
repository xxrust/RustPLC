...
### 4.7 `05_supervision/`、`06_manual/`、`07_hmi/`

这三层不是“待补完的生产主流程”，而是结构化项目里预留的运行层。

- `05_supervision/`：模式仲裁、启动/停止、自动循环使能、front-door 协议。
- `06_manual/`：人工维护与手动操作。
- `07_hmi/`：HMI 状态、报警、提示与人机交互。

规则：

- 默认禁用不等于空白失效，而是表示该层当前未作为主交付面启用。
- 这些层不应和 `02_process/` 混写成一个大 task。
- 如果项目当前只交付自动主流程，可以先保留目录和说明，但必须在 README / wiki 里讲清楚其组织意图。
