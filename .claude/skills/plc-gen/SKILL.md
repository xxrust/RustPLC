...
## Hard Guardrail: State Must Be Feedback-Proved

Machine state is sensor-backed unless the system contract explicitly says the step is no-feedback.
Do not assume any physical action succeeded by default.

Treat these as hard modeling smells during generation or repair:
- `variable ...: bool = true` used to let a production step pass before any field proof exists
- `wait: <internal_flag> == true` when that flag is not derived from a sensor, operator command, workpiece token, or explicitly no-feedback action
- a cassette, tray, bin, rack, magazine, buffer, or holder modeled as available only because it is an `ingress_site`
- a normal path that continues after an action without sensor proof, topology-closed action completion, timeout/fault routing, or an explicit no-feedback note

For finite supply, distinguish:
- physical availability proof, such as a cassette-present sensor, wafer-present sensor, vacuum confirmation, window sensor, or upstream handoff signal
- workpiece token location and capacity
- test-only scenario seeding

Do not let a test-only seed become production truth.
If the proof source is missing, report a blocker or repair the model so the state is proven before the step can leave.

## Hard Guardrail: Indefinite Wait Is Only For Uncontrolled Others

Use `allow_indefinite_wait: true` only when the current task is waiting for an actor or task it does not control:
- operator front-door commands such as start, reset, acknowledge, manual confirmation, or HMI command
- upstream/downstream handoff signals such as request, ready, allow, available, or done owned by another station/task
- deliberately external process readiness that cannot be forced by this task

Do not use `allow_indefinite_wait: true` for the current task's own feedback:
- local sensors such as `sensor_*`, `wafer_on_*`, home, limit, empty, drop, in-position, vacuum-ok
- feedback that should be caused by an action in the same task
- residual/empty checks during startup

Those waits need a timeout plus explicit recovery/fault routing, or a higher-level topology-closed device action. If the route requires human help, name that boundary explicitly with a manual/operator-assist task; do not hide it behind a generic startup fault.
...
11. When asked to `fix`, first search for unproven success assumptions and sensor-proof gaps before polishing syntax or scenarios.
...
7. If the request is a repair or `fix`, run the fix loop from `references/commands.md` before declaring the source healthy.
8. Run `project-check`.
9. Confirm that `intent_alignment` actually appeared as a step.
10. Report the real verdict, mismatch, or blocker.
