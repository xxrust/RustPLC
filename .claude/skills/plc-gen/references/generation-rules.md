...
Every generated or repaired production state must have an evidence source.
Default success is not an evidence source.

Before writing a `wait` or an internal readiness flag, identify one of:
- field sensor / controller input evidence
- topology-closed semantic action completion
- workpiece token transition
- operator front-door event
- explicit no-feedback step documented by the system contract

Do not generate internal booleans such as `*_has_seed`, `*_ready`, or `*_done` with an initial `true` value just to let the first cycle pass.
If the real machine state depends on a cassette, tray, magazine, buffer, rack, holder, vacuum cup, or physical window, model the sensor or upstream handoff that proves it.
If that proof is missing, write a blocker or a fault/timeout path; do not silently assume the action completed.
...
- 只有等待不受本 task 控制的他者时才允许 `allow_indefinite_wait: true`，例如 operator front-door、上游 task handoff、下游 ready/request；本 task 自己的传感器、真空反馈、home/limit/empty/drop 等本地反馈不得无限等待
...
- 非外部他者 wait 默认应有 timeout、恢复路径或显式 fault route
