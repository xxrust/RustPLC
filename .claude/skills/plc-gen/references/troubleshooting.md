...
### Unproven success state / test seed leaked into production logic
...
Likely cause:
- a nominal one-piece test seed was treated as physical machine truth
- the production step waits on an internal flag instead of a sensor-backed state
- `ingress_sites` was confused with verified cassette inventory or upstream feed success

Fix:
- trace the flag to its proof source before editing
- if the state should be physical, model a field sensor, upstream handoff, vacuum confirmation, or operator/front-door event that proves it
- if the step is intentionally no-feedback, mark that explicitly in the system contract and keep timeout/fault behavior honest
- if proof is missing, report a blocker; do not initialize the flag to `true` or set it back to `true` to make the cycle pass

### Indefinite wait used for a local feedback sensor
...
Likely cause:
- the generator treated all long waits as manual/operator waits
- the current task's own sensor fault was hidden as an unbounded wait

Fix:
- keep `allow_indefinite_wait` only for uncontrolled others: operator commands, upstream/downstream handoff, or external process readiness
- for local feedback, add timeout and route to automatic recovery or an explicitly named manual/operator-assist task
- if the wait is really produced by a topology-closed actuator action, model the action result instead of hand-writing the sensor wait
